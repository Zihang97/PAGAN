...
class dataset_preprocess_FTGAN(Dataset):
    ...
    def __getitem__(self, item):
        ...
        clean_wav = np.load(os.path.join(self.clean_wav_npy, selected_tf))
        noisy_wav = np.load(os.path.join(self.noisy_wav_npy, selected_tf))
        clean_tf = np.load(os.path.join(self.clean_tf, selected_tf))
        noisy_tf = np.load(os.path.join(self.noisy_tf, selected_tf))
        noisy_phase = np.load(os.path.join(self.noisy_phase, selected_tf))

        clean_wav = torch.Tensor(clean_wav)
        noisy_wav = torch.Tensor(noisy_wav)
        clean_tf = torch.Tensor(clean_tf)
        noisy_tf = torch.Tensor(noisy_tf)
        noisy_phase = torch.Tensor(noisy_phase)

        return clean_tf, noisy_tf, clean_wav, noisy_wav, noisy_phase
...
def train():
    ...
    for e in range(1000):
        ...
        for num, (clean_tf, noisy_tf, clean_wav, noisy_wav, noisy_phase) in enumerate(train_loader):
            # print('num:', num)
        ...
        if (e + 1) % 1 == 0:
            ...
            G.eval()
            G.cpu()
            model_mid_path_G = os.path.join('/workspace/model/rgan_SN_2/G', model_mid_name)
            torch.save(G.state_dict(), model_mid_path_G)
            D.eval()
            D.cpu()
            model_mid_path_D = os.path.join('/workspace/model/rgan_SN_2/D', model_mid_name)
